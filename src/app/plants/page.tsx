...
export default function PlantsPage() {
  ...
  const [view, setView]       = useState<View>('toc')      // mặc định mục lục
  const [currentIdx, setCurrentIdx] = useState(0)
  const [activeImg, setActiveImg]   = useState<'cover'|'flower'|'app'>('cover')
  const [showSidebar, setShowSidebar] = useState(false)    // sidebar mục lục khi đang ở catalog
  ...
  // keyboard nav (chỉ khi đang ở catalog)
  useEffect(() => {
  ...
  // click vào cây từ mục lục → vào catalog từ vị trí đó
  const goToPlant = (idx: number) => {
    setCurrentIdx(idx)
    setActiveImg('cover')
    setView('catalog')
  ...
  const Topbar = () => (
    <div className="flex-shrink-0 bg-[#0d1109] border-b border-[#2a2e24] px-4 py-2 flex items-center justify-between z-30">
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={() => view === 'toc' ? null : setShowSidebar(!showSidebar)}
          className={`flex items-center gap-1.5 text-sm transition-colors ${view === 'catalog' ? 'text-gray-400 hover:text-white cursor-pointer' : 'text-white'}`}
        >
          <List size={15} />
          <span>Mục Lục</span>
        </button>
              ...
              {gPlants.map((p, pi) => {
                ...
                return (
                  <button key={p.id} onClick={() => goToPlant(globalIdx)}
                    className={`w-full flex items-center justify-between px-4 py-1.5 text-left transition-colors hover:bg-forest-50 ${globalIdx === currentIdx && view === 'catalog' ? 'bg-forest-50 text-forest-700' : ''}`}>
                    <span className="text-sm text-gray-600">
                      <span className="text-gray-300 mr-2 text-xs">{pi + 1}.</span>{p.name_vi}
                    </span>
            ...
            return (
              <div key={g1.id}>
                <div className="flex items-baseline gap-4 mb-3">
                  <span className="text-xs font-bold text-[#2d6a30] bg-[#d4ebbb] px-2 py-0.5 rounded">
                    {String(gi + 1).padStart(2, '0')}
                  </span>
                  ...
                  {shown.map((p, pi) => {
                    ...
                    return (
                      <button key={p.id} onClick={() => goToPlant(globalIdx)}
                        ...
                        <span className="text-xs italic text-[#9a9a82] ml-4">{p.scientific_name}</span>
                      </button>
                    )
                  })}
                  {rest > 0 && (
        ...
        <>
          ...
          <div className="w-[52%] flex-shrink-0 relative overflow-hidden">
            ...
            <div className="absolute top-4 right-4 flex flex-col gap-2">
              {[
                { key: 'cover' as const, label: 'Tổng thể', url: current.cover_image_url },
                { key: 'flower' as const, label: 'Hoa / lá', url: current.flower_leaf_image_url },
                { key: 'app' as const, label: 'Ứng dụng', url: current.application_image_url },
              ].map(({ key, label, url }) => (
